...
func _ready():
	...
	if GlobalScene.currentgame==4:check_mashbash()
...
func check_mashbash():
	var completed_scores = 0
	for score in GlobalScene.scoreBoard[4]:
		if score==1:
			completed_scores+=1
	$ViewportContainer/Viewport/Labels/HighScores.text = "Endings Reached:\n"+str(completed_scores)+"/"+str(GlobalScene.scoreBoard[4].size())
