...
func final_level_text():
	var set = [24,25,26,27,28]
	var selected_ending = 0
	...
	if GlobalScene.triggered_annoyances >=3:
		...
		if GlobalScene.cake:
			selected_ending = 3
		if GlobalScene.entered_top_area:
			selected_ending = 4
	get_tree().get_nodes_in_group("BottomText")[0].currentset = set[selected_ending]
	...
	#allows the endings to increase in number
	while GlobalScene.scoreBoard[4].size() < selected_ending-1:
		GlobalScene.scoreBoard[4].append(0)
	GlobalScene.scoreBoard[4][selected_ending]=1
